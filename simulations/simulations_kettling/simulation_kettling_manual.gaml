...
model simulationkettling



import "../../models/unanimate/building.gaml"
import "./../common_simulation.gaml"
import "../../models/protest_configuration/kettling.gaml"
...
global {
	...
	init {
		write "global simulation_kettling "+name;
		...
		map<string,unknown> sent_config_params <- [
				...
				"P_kettling_n_ortho_cops"::P_kettling_n_ortho_cops,
				"P_kettling_n_transverse_cops"::P_kettling_n_transverse_cops,
				"P_kettling_dmax_cop_line_ortho"::P_kettling_dmax_cop_line_ortho
			];
...
experiment kettling_manual type: gui { // 4h simulation
    // Pedestrian params
	...
	parameter "dist to position team max" var:P_dmax_my_position_team init: 20.0#m; // !! view dist, could be divided by two or more
	...
	parameter "speed of teams" var: P_team_speed init:1.3#m/#s; // !! try with 1.3m.s-1, take enough time for officers to follow OK
	...
	parameter "Half max lateral distance for line formation" var: P_half_dmax_lateral init:2#m; // !! should depend on protest config and nbr police officer
	...
	parameter "Rioter energy consumption when retreating" var: P_rioter_energy_retreat_consumption init:1/step; // fixed can run 5 min with full energy = energy_init/(5*60)
	...
	// Kettling protest configuration
	parameter "Nb cops ortho" var:P_kettling_n_ortho_cops init:1;
	parameter "Nb cops transverse" var:P_kettling_n_transverse_cops init:12;
	parameter "Line formation dmax" var:P_kettling_dmax_cop_line_ortho init:2#m;
	...
	output {
		display main_display type:2d antialias:false {
			...
			species breakable_item aspect: item_base;
			//species rioter aspect: show_negative_emotions;
			species rioter aspect: state;
			//species rioter aspect: connections_emotional_contagion;
		...
		display Population_information refresh: every(1#cycles)  type: 2d {
			...
			chart "Events detection" type:series size:{0.5,1.0} position: {0.5,0.0}{
				...
				data "spatial_incursion detected" value: rioter count (each.is_event_detected["spatial_incursion"]) color:#blue;
			}
		}
		...
		display Population_information_violence refresh: every(1#cycles)  type: 2d {
			chart "Violence" type: series size: {1.0,0.5} position: {0.0, 0.0} {
				...
				data "ratio retreat" value: (rioter count (each.is_in_state["retreat"]))/length(rioter) color:#green;
			}
			
			chart "N arrests" type: series size: {1.0,0.5} position: {0, 0.5} {
				data "N arrested" value: 200 - length(rioter) color:#red;
				data "N arrest team" value: length(arrest_team) color:#blue;
			}
		}
	}
}
